(function() {
    ...
    var numberOfCores = require('os').cpus().length,
        ...
        extend = require('./tools/object_extend');
    ...
    function Ranch(options, path) {
        this.options = extend(DEFAULT_CONFIG, options);
        this.path = path;
        this.activeCalls = 0;
    }
    ...
    Ranch.prototype.mkhandle = function(method) {
        return function() {
    ...
    // # Some sort of initiator.
    Ranch.prototype.setup = function(methods) {
        var iface
    ...
    Ranch.prototype.onExit = function(childId) {
    ...
    // # # start a new worker
    Ranch.prototype.startChild = function() {
        this.childId++
    ...
    Ranch.prototype.stopChild = function(childId) {
    ...
    Ranch.prototype.receive = function(data) {
        var idx = data.idx,
            childId = data.child,
            args = data.args,
        ...
        if (!child) {
            return console.error(
                'Worker Ranch: Received message for unknown child. ' + 'This is likely as a result of premature child death, ' + 'the operation will have been re-queued.'
            )
        }
        ...
        if (!call) {
            return console.error(
                'Worker Ranch: Received message for unknown index for existing child. ' + 'This should not happen!'
            )
        }
    ...
    Ranch.prototype.childTimeout = function(childId) {
        var child = this.children[childId],
            i
    ...
    Ranch.prototype.send = function(childId, call) {
        var child = this.children[childId],
    ...
    Ranch.prototype.childKeys = function() {
    ...
    Ranch.prototype.processQueue = function() {
        var cka, i = 0,
            childId
    ...
    Ranch.prototype.addCall = function(call) {
        if (this.ending)
    ...
    Ranch.prototype.end = function(callback) {
        var complete = true
